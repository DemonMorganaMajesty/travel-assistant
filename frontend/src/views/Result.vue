<template>
  ...
  <div class="result-container">
    ...
    <!-- 未登录降级提示：仅一次性使用，不持久化 -->
    <div v-if="!isLoggedIn" class="anon-result-tip">
      ⚠️ 未登录：当前方案仅一次性使用，不能保存历史行程、不能持久化用户偏好记忆、不能保存聊天伴游历史。
    </div>

    <!-- 无效行程数据时的空状态（防止白屏） -->
    <div v-if="tripPlan && !hasValidPlan" class="result-empty">
      <a-empty description="暂无有效的行程数据，请返回首页重新规划" />
      <a-button type="primary" @click="goBack">← 返回首页重新规划</a-button>
    </div>

    <!-- 方案切换条：方案一/方案二/方案三 自由切换 -->
    <div v-if="planMode && planList.length > 1" class="plan-switcher">
      <span class="plan-switcher-label">📊 选择方案:</span>
      <a-radio-group v-model:value="activePlanType" button-style="solid" @change="onSwitchPlan">
        <a-radio-button v-for="p in planList" :key="p.plan_type" :value="p.plan_type">
          {{ p.plan_name }}{{ planMetricsText(p) }}
        </a-radio-button>
      </a-radio-group>
      <span class="plan-switcher-desc">{{ currentPlanDesc }}</span>
    </div>



    <div v-if="tripPlan && hasValidPlan" class="content-wrapper">
          ...
          <a-menu mode="inline" :selected-keys="[activeSection]" @click="scrollToSection">
            ...
            <a-sub-menu key="days" title="📅 每日行程">
              <a-menu-item v-for="(day, index) in tripPlan.days" :key="`day-${index}`">
                第{{ day.day_index + 1 }}天 {{ day.city_name }}
              </a-menu-item>
            </a-sub-menu>
      ...
      <div class="main-content">
        ...
        <div class="top-info-section">
          ...
          <div class="left-info">
            ...
            <a-card id="overview"
                    :title="tripPlan.city_list ? '多城市联合旅行计划' : `${tripPlan.city}旅行计划`"
                    :bordered="false" class="overview-card">
              <div class="overview-content">
                <div class="info-item">
                  <span class="info-label">📅 日期:</span>
                  <span class="info-value">{{ tripPlan.start_date }} 至 {{ tripPlan.end_date }}</span>
                </div>
                <div v-if="tripPlan.origin" class="info-item">
                  <span class="info-label">🚗 出发地:</span>
                  <span class="info-value">{{ tripPlan.origin }}（返程回到出发地）</span>
                </div>
                <div v-if="tripPlan.city_list" class="info-item">
                  <span class="info-label">🏙️ 涉及城市:</span>
                  <span class="info-value">
        {{ tripPlan.city_list.map(c => c.city_name).join(' / ') }}
      </span>
                </div>
                <div class="info-item">
          ...
          <a-card id="map" title="📍 景点地图" :bordered="false" class="map-card">
              ...
              <div id="amap-container" style="width: 100%; height: 480px"></div>
            ...
            >
              ...
              <template #header>
                <div class="day-header">
                  <span class="day-title">第{{ day.day_index + 1 }}天 · {{ day.city_name }}</span>
...
<!--    <ChatWidget v-if="tripPlan" :plan="tripPlan" />-->
...
import type { TripPlan, PlanItem, TripPlanMulti } from '@/types'
import { STORAGE_KEYS, findActivePlan, DEFAULT_PLAN_TYPE } from '@/constants'



const router = useRouter()

// 登录态：未登录时结果仅一次性使用，不持久化
...
// 行程数据合法性校验：防止 sessionStorage 中残留无效数据导致白屏
const hasValidPlan = computed(() => {
  const p = tripPlan.value
  return !!(p && typeof p === 'object' && p.city && Array.isArray(p.days) && p.days.length > 0)
})

// 方案指标展示：总通勤时间 / 总花费（由后端按真实数据计算）
const planMetricsText = (p: PlanItem) => {
  const m = p.plan_metrics
  if (!m || m.commute_minutes === undefined || m.total_cost === undefined) return ''
  return ` ⏱${m.commute_minutes}min · ¥${m.total_cost}`
}

// 当前方案描述（切换条右侧提示）
const currentPlanDesc = computed(() => {
  const found = planList.value.find((p) => p.plan_type === activePlanType.value)
  return found?.plan_desc || ''
})
...
onMounted(async () => {
  ...
  if (multiRaw) {
    try {
      const multi: TripPlanMulti = JSON.parse(multiRaw)
      planList.value = multi.plans || []
      activePlanType.value = multi.active_plan_type || DEFAULT_PLAN_TYPE
      planMode.value = planList.value.length > 0
      const active = findActivePlan(planList.value, activePlanType.value)
      if (active) {
        tripPlan.value = active.plan
      }
    } catch (e) {
      console.error('Parse tripPlanMulti failed:', e)
    }
  }

  const data = sessionStorage.getItem(STORAGE_KEYS.tripPlan)
  console.log('Result mounted, tripPlan in storage:', !!data)

  if (!tripPlan.value && data) {
    tripPlan.value = JSON.parse(data)
  }

  if (tripPlan.value && hasValidPlan.value) {
    console.log('Parsed tripPlan:', tripPlan.value?.city, tripPlan.value?.days?.length, 'days')
...
// 切换三方案：更新当前渲染的 tripPlan，并同步 sessionStorage（伴游对话使用）
const onSwitchPlan = () => {
  const active = findActivePlan(planList.value, activePlanType.value)
  if (!active) return
  tripPlan.value = active.plan
  sessionStorage.setItem(STORAGE_KEYS.tripPlan, JSON.stringify(active.plan))
  // 同步更新三方案存储中的 active_plan_type
  const multiRaw = sessionStorage.getItem(STORAGE_KEYS.tripPlanMulti)
  if (multiRaw) {
    try {
      const multi: TripPlanMulti = JSON.parse(multiRaw)
      multi.active_plan_type = activePlanType.value
      sessionStorage.setItem(STORAGE_KEYS.tripPlanMulti, JSON.stringify(multi))
    } catch (e) {
      console.error('Update tripPlanMulti failed:', e)
    }
  }
  message.success(`已切换为「${active.plan_name}」方案`)
  // 重新加载图片与地图，反映新方案内容
...
const loadAttractionPhotos = () => {

  if (!tripPlan.value) return

  // 高德图片接口只支持单个城市：多城市时按天取 day.city_name，缺失时回退到城市列表第一个
  const fallbackCity = (tripPlan.value.city || '').split(',')[0].trim()

  const promises: Promise<void>[] = []

  tripPlan.value.days.forEach(day => {

    // 单日城市：优先用 day.city_name（多城市场景），否则回退到第一个城市
    const dayCity = (day.city_name || fallbackCity || '').trim()
    const city = encodeURIComponent(dayCity)

    day.attractions.forEach(attraction => {

      const name = encodeURIComponent(attraction.name)
      const apiUrl = `/api/poi/amap-photo?name=${name}&city=${city}`
      ...
      const p = fetch(apiUrl)
        ...
        .then(data => {
          if (data?.success && data?.data?.photo_url) {
            ...
            console.log('Photo found:', attraction.name, data.data.photo_url?.slice(0, 60))
          } else {
            console.log('No photo for:', attraction.name, data)
          }
        })
        .catch((err) => { console.error('Fetch error:', attraction.name, err) })
  ...
  Promise.all(promises).then(() => {
    console.log(`Loaded photos for ${promises.length} attractions`)
  })
}
...
const getAttractionImage = (name: string, index: number): string => {
  ...
  // 简单纯色占位图（避免中文在 data URI 中的编码问题）
  const svgPlain = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300">
    <defs>
  ...
  return `data:image/svg+xml,${encodeURIComponent(svgPlain)}`
...
const initMap = async () => {
    ...
    // 动态加载高德地图 JS API（绕过 @amap/amap-jsapi-loader 的兼容性问题）

    const loadAmap = (key: string): Promise<any> => {
      ...
      return new Promise((resolve, reject) => {
        ...
        script.src = `https://webapi.amap.com/maps?v=2.0&key=${key}&plugin=AMap.Marker,AMap.Polyline,AMap.InfoWindow`
    ...
    try {
      ...
      const key = import.meta.env.VITE_AMAP_WEB_JS_KEY as string
...
const addAttractionMarkers = (AMap: any) => {
  ...
  tripPlan.value.days.forEach((day, dayIndex) => {
    ...
    day.attractions.forEach((attraction, attrIndex) => {
      ...
      // 验证坐标是否为有效数字（非 NaN、null、0 或字符串 "NaN"）
